...
module RBI
  class CleanTreeTest < Minitest::Test
    def test_clean_tree_does_nothing_with_an_empty_index
      index = Index.new
      ...
      cleaned, operations = Rewriters::CleanTree.clean(original, index)

      assert_equal(shim, cleaned.string)
      assert_empty(operations)
    end

    def test_clean_tree_removes_all_definitions_found_in_index
      ...
      cleaned, operations = Rewriters::CleanTree.clean(shim, index)
    ...
    def test_clean_tree_keeps_definitions_not_found_in_index
    ...
    def test_clean_tree_removes_empty_scopes
      tree1 = Parser.parse_string(<<~RBI)
        class Foo
          attr_reader :foo
        end
    ...
    def test_clean_tree_keeps_empty_scopes_not_found_in_index
      tree1 = Parser.parse_string(<<~RBI)
        class Foo; end
      RBI
